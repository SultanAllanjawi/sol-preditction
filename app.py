...
with tab9:
    ...
    st.caption("Ask anything about this asset, signals, portfolio, or trading. Powered by Claude AI.")

    _ai_price = f"${display_price:,.4f}"
    _ai_tp    = f"${tp_price:,.4f}" if tp_price else "N/A"
    _ai_sl    = f"${sl_price:,.4f}" if sl_price else "N/A"
    _ai_conf  = f"{last_conf:.1f}%"
    _ai_pup   = f"{last_prob*100:.1f}%"
    _ai_accf  = f"{ens_filt*100:.1f}%"
    _ai_accr  = f"{ens_acc*100:.1f}%"
    _ai_rr    = f"1:{rr:.2f}"
    ...
    _port_str = "\n".join(_port_lines) if _port_lines else "  None"

    _sig_lines = []
    try:
        for _, _sr in sig_hist.head(5).iterrows():
            _sl_d = str(_sr.get("Date",""))[:10]
            _sl_s = str(_sr.get("Signal",""))
            _sl_p = str(_sr.get("Price",""))
            _sl_c = str(_sr.get("Confidence",""))
            _sig_lines.append(f"  - {_sl_d} {_sl_s} @ {_sl_p} ({_sl_c})")
    except Exception:
        pass
    _sig_str = "\n".join(_sig_lines) if _sig_lines else "  None"

    _sys_lines = [
        "You are an expert AI trading assistant embedded in a professional ML trading dashboard.",
        "",
        "LIVE DASHBOARD DATA:",
        f"Asset:          {name} ({ticker})",
        f"Live Price:     {_ai_price}",
        f"ML Signal:      {last_sig}",
        f"Confidence:     {_ai_conf}",
        f"P(UP):          {_ai_pup}",
        f"Accuracy:       {_ai_accf} filtered / {_ai_accr} raw",
        f"Take Profit:    {_ai_tp}",
        f"Stop Loss:      {_ai_sl}",
        f"R/R:            {_ai_rr}",
        f"ATR:            {_ai_atr}",
        f"Mode:           {_ai_mode}",
        ...
        "",
        "INSTRUCTIONS:",
        "- Answer ALL questions accurately using the live data above",
        "- For signal questions: give exact signal, entry, TP, SL",
        "- For strategy: specific actionable advice based on the data",
        "- Be concise and direct",
        "- Mention 'not financial advice' only once per session",
        f"- You only have live data for {name} right now",
    ]
    _sys_ctx = "\n".join(_sys_lines)

    _chat_key = f"ai_msgs_{ticker}"
    if _chat_key not in st.session_state:
        st.session_state[_chat_key] = []

    _qb1, _qb2, _qb3, _qb4, _qb5 = st.columns([2,2,2,2,1])
    _quick_map = {
        _qb1: "Is there a signal right now?",
        _qb2: "What are the TP and SL?",
        _qb3: "Should I enter this trade?",
        _qb4: "Explain the model accuracy",
    }
    for _col, _q in _quick_map.items():
        if _col.button(_q, key=f"aiq_{abs(hash(_q))}", use_container_width=True):
            st.session_state[_chat_key].append({"role": "user", "content": _q})
            st.rerun()
    if _qb5.button("🗑", key="ai_clr", use_container_width=True, help="Clear chat"):
        st.session_state[_chat_key] = []
        st.rerun()
    ...
    _user_q = st.chat_input(f"Ask anything about {name} or trading...", key=f"ai_inp_{ticker}")
            ...
            with st.spinner(""):
                try:
                    ...
                    _msgs_api = [
                        ...
                        for m in st.session_state[_chat_key][-12:]
                    ]
                    _resp = _rq.post(
                    ...
                    if _resp.status_code == 200:
                        _ans = _resp.json()["content"][0]["text"]
                        st.markdown(_ans)
                        st.session_state[_chat_key].append({"role": "assistant", "content": _ans})
                    else:
                        _e = f"API error {_resp.status_code}: {_resp.text[:100]}"
                        st.error(_e)
                        st.session_state[_chat_key].append({"role": "assistant", "content": _e})
                except Exception as _ex:
                    _e = f"Connection error: {str(_ex)[:150]}"
                    st.error(_e)
                    st.session_state[_chat_key].append({"role": "assistant", "content": _e})
