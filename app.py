...
<div style="background:#1C2128;border:2px solid #E3B341;border-radius:12px;
  ...
  <div style="color:#6E7681;font-size:0.80rem">
    ...
    day_chg       = 0.0; last_close = display_price
    ens_proba     = _np2.array([0.5]); ens_pred = _np2.array([0])
    y_te          = _np2.array([0]);   signals  = _np2.array([0])
    price_pred    = _np2.array([display_price])
    y_price_te    = _np2.array([display_price])
    model_data    = {};  sig_hist = _pd2.DataFrame()
    te_df         = _pd2.DataFrame()
    ...
    entry_p = display_price; name = ticker; n = 1
    _next_date     = _nd.date() + timedelta(days=1)
    next_str       = _next_date.strftime('%A %d %b %Y')
    next_str_short = _next_date.strftime('%a %d %b')
